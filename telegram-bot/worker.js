...
// Keywords that mark a message as likely congratulating someone (birthday,
// promotion, anniversary, a win, etc.) — see maybeJoinCongrats().
const CONGRATS_KEYWORDS = [
  "вітаю", "вітаємо", "поздоровля", "день народження", "днем народження",
  "ювіле", "роковин", "з нагоди", "вітання", "підвищенням", "призначенням", "перемогою",
];
...
Приєднання до привітань (адміни чату, потребує ANTHROPIC_API_KEY):
...
// anniversary, a win...), the bot joins in with its own short, warm reply —
// generated by Claude so it actually reacts to what was written, instead of
// a canned template. Needs the ANTHROPIC_API_KEY secret; silently does
// nothing without it. /congrats on|off toggles it per chat (default on).
...
function looksLikeCongrats(text) {
  if (!text) return false;
  const lower = text.toLowerCase();
  return CONGRATS_KEYWORDS.some((kw) => lower.includes(kw));
}

async function maybeJoinCongrats(chatId, msg, env) {
  if (!env.ANTHROPIC_API_KEY) return;
  if (!looksLikeCongrats(msg.text)) return;
  ...
  const replyText = await generateCongratsReply(msg.text, env);
  if (!replyText) return;
...
async function generateCongratsReply(sourceText, env) {
  try {
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        system:
          "Ти — дружній бот команди роздрібної мережі JYSK у Telegram-чаті дістрикту Kyiv-1. Хтось у чаті щойно привітав колегу з якоюсь подією (день народження, підвищення, ювілей, перемога тощо). Приєднайся до привітання ОДНИМ коротким теплим повідомленням українською (1-2 речення, можна з emoji), яке звучить природно і стосується саме цієї події з тексту нижче. Не повторюй дослівно те, що вже написали. У відповіді — лише текст привітання, без пояснень.",
    ...
    console.error("generateCongratsReply error", err);
    return null;
  }
}
