...
const STORAGE_KEY = 'district-tracker-data';
...
export default function StoreTracker() {
    ...
    (async () => {
      try {
        ...
        if (res && res.value) {
          ...
          if (data.stores && data.stores.length) setStores(data.stores);
          if (data.kpis) setKpis(data.kpis);
          if (data.entries) setEntries(data.entries);
          if (data.kpis && data.kpis.length) setActiveKpiId(data.kpis[0].id);
        }
      } catch (e) {
  ...
  useEffect(() => {
    if (!loaded) return;
    persist({ stores, kpis, entries });
  }, [stores, kpis, entries, loaded, persist]);
  ...
  const removeKpi = (id) => {
    ...
    if (activeKpiId === id) {
      const rest = kpis.filter((k) => k.id !== id);
      setActiveKpiId(rest.length ? rest[0].id : null);
    }
  };
  ...
  const updateStoreName = (code, name) => {
    setStores((prev) => prev.map((s) => (s.code === code ? { ...s, name } : s)));
  };

  const updateStoreCode = (oldCode, newCode) => {
    const trimmed = newCode.trim();
    if (!trimmed || trimmed === oldCode) return;
    setStores((prev) => prev.map((s) => (s.code === oldCode ? { ...s, code: trimmed } : s)));
    setEntries((prev) => {
      const next = {};
  ...
  const addStore = () => {
    ...
    setStores((prev) => [...prev, { code, name: 'Новий магазин' }]);
  };
  ...
  return (
    <div className="w-full bg-white text-neutral-900 font-sans">
      <div className="border-b border-neutral-200 px-6 py-4 flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-lg font-semibold tracking-tight">Трекер показників дістрікту</h1>
          <p className="text-xs text-neutral-500 mt-0.5">{stores.length} магазинів · {kpis.length} показників</p>
        </div>
      ...
      {kpis.length === 0 ? (
        ...
        <>
          <div className="px-6 py-4 grid grid-cols-3 gap-3 border-b border-neutral-200 bg-neutral-50">
